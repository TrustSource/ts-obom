...
### ADR-009 - A deployment is named in the document, and only means something with its parameters (2026-09-24, 0.3.0)

**Context.** One project is deployed several times: environments (`DEV`, `PRD`) and customer setups (`kunde1`, `kunde2`). Putting those side by side is what makes an OBOM useful for drift and for threat modelling -- a finding that exists only in production is a different statement from a finding that exists. CycloneDX has no field for "which deployment this BOM describes": checked against the 1.6 schema, the only `deployment`/`environment` occurrences belong to machine-learning lifecycles and the environmental footprint.

The trap is that in the pattern this is built for -- **one template, one parameter file per environment**, which is how the TrustSource repositories themselves deploy (`deploy2DEV.sh` / `deploy2PRD.sh` feeding `params4DEV.json` / `params4PRD.json` into the same template) -- the whole difference between two deployments lives in the parameters. And the scan does not read them: it resolves `Ref`/`!Sub` against the template's own `Default`, verified empirically. Two runs over the same sources therefore produce byte-identical documents. Labelling one `DEV` and one `PRD` would make them look comparable and show "no drift", which would be an artefact rather than a finding.

**Decision.** The deployment name is a property of the document -- `metadata.properties[] trustsource:obom:deployment` -- not a component, not a lifecycle phase, and not an upload parameter. It is what the `--deployment` option writes, so the argument travels through unchanged and document and storage cannot contradict each other.

It is always accompanied by `trustsource:obom:parameterSource`, which is **never omitted**: either `defaults`, or `<front-end>=<file>` per active front-end. Naming a deployment without supplying parameter values is allowed but warned about, at scan time and again at upload time. `metadata.lifecycles` declares `phase: operations` alongside the TrustSource marker, so a CycloneDX consumer that knows nothing about TrustSource can still tell what kind of BOM this is.

**Consequences.** Two documents are comparable exactly when both carry a real parameter source; `defaults` on both sides means their equality is meaningless, and a consumer can see that without guessing. The platform gains a dimension it did not have -- `Obom.findLatest` returns one document per scope, so two deployments would shadow each other; that is tracked as eacg-gmbh/ts-app#173 and is what makes the naming usable end to end.

Terraform takes parameter values today (`--terraform:var-file`, applied through the parser's own `vars_files` hook). CloudFormation does not yet: checkov's parser accepts no external parameter file, so a CFN scan always records `defaults`. That is the honest half-state, and it is visible in every document rather than hidden -- the second half is the next step.

## Further development

Backlog, in rough order:

1. **CloudFormation parameter files** (ADR-009). `--cloudformation:parameters params4PRD.json` in TrustSource's own parameter format (`[{ParameterKey, ParameterValue}]`), applied on top of the template defaults before the graph is built. Until then every CloudFormation scan records `parameterSource=defaults`, and a named deployment describes the template's defaults rather than that deployment's values.
...
3. **CIS benchmarks as an optional extra** (ADR-008, not scheduled). A `ts-obom[checks]` install would depend on the real `checkov` package, run a benchmark subset over the sources already scanned, and hand the findings to the exporter as `failed_checks` so they land as `vulnerabilities` on the components the OBOM already carries. Everything on the ts-obom side is in place for that -- `_InventoryReport` has the fields, checkov's exporter does the rest. The work that decides whether it is worth doing is on the platform: reading those findings out of a project-scope OBOM, showing them in the project view, and giving them a way into risk management with an identity derived from project and deployment (ADR-009), so the same finding in two environments stays two findings.
4. `AWS::IAM::Policy` / `AWS::IAM::ManagedPolicy` resources and Terraform `aws_iam_policy` documents referenced by attachments.
5. Resource based policies (bucket, queue and topic policies).
6. A Kubernetes RBAC front-end.
