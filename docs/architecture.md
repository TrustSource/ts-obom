...
Both front-ends take parameter values. Terraform through the parser's own `vars_files` hook (`--terraform:var-file`); CloudFormation by overriding the templates' parameter `Default` values before the graph is built (`--cloudformation:parameters`), because checkov's parser accepts no external parameter file and the graph resolves `Ref`/`!Sub` against those defaults. Both TrustSource's own `[{ParameterKey, ParameterValue}]` format and a flat name/value mapping are read, told apart by their parsed shape rather than by a flag. A value supplied for a parameter no template declares is reported as an `unused-parameter` entry under `unresolved`, because silently ignoring it would let `parameterSource` claim more than the scan applied.
