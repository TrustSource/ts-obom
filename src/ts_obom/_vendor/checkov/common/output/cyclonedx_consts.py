...
# ts-obom: see the same edit in cyclonedx.py -- avoids importing report.py.
...
# ts-obom: default raised from 1.4 to 1.6 and the two newer versions added to the
# map. 1.6 is what TrustSource's OBOM API is specified against, and the component
# types an Operations BOM needs (`platform`, `data`) only exist from 1.5 on.
DEFAULT_CYCLONE_SCHEMA_VERSION = SchemaVersion.V1_6

CYCLONE_SCHEMA_VERSION: "dict[str, SchemaVersion]" = {
    "1.6": DEFAULT_CYCLONE_SCHEMA_VERSION,
